...
const float halfpi = 1.62;

//  Model coordinates and light from vertex shader
varying float LightIntensity;
varying vec2  ModelPos;

// rotation and mode of "tower"
uniform float rotation;

// mapped rect position
varying vec2 mapped_pos;

/*
 * This will be called to "map" the brick size based on position in the object
 * Takes the size to be altered and returns the new
*/
vec2 cyl(vec2 size) {
  float ratio = sqrt(1 - pow(mapped_pos.x, 2));
  return size * ratio + .3;
}

void main()
{
  // get radial size of brick
  vec2 RadBrickSize = cyl(BrickSize);
  //  Divide by brick size for unit coordinates
  vec2 position = ModelPos / RadBrickSize;
  //  Stagger rows
  if (fract(0.5*position.y)>0.5) position.x += 0.5;
  //  Scroll bricks at .5 bricks per rotation
