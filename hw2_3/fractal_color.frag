...
const vec4 fractColor = vec4(.8,.0,.0,1.0);
...
// returns 1.0 if p.x *AND* p.y are in range r
float inRange_and(vec2 p, vec2 r) {
...
void main() {
  ...
  for(int n=0; n<iterations; n++) {
    if(inRange_and(fract(coords*fractal_div), center) == 1.0) {
      gl_FragColor = fractColor;
    }
